-- First, clean up duplicate locations by keeping the oldest record and updating references
DO $$
DECLARE
  duplicate_record RECORD;
  keeper_id uuid;
BEGIN
  -- Find and resolve duplicate locations (case-insensitive)
  FOR duplicate_record IN
    SELECT lower(name) as name_lower, array_agg(id ORDER BY created_at) as ids
    FROM public.locations
    GROUP BY lower(name)
    HAVING count(*) > 1
  LOOP
    -- Keep the first (oldest) location
    keeper_id := duplicate_record.ids[1];
    
    -- Update any facts_monthly_location records pointing to duplicates
    UPDATE public.facts_monthly_location 
    SET location_id = keeper_id
    WHERE location_id = ANY(duplicate_record.ids[2:]);
    
    -- Update any transactions pointing to duplicates
...
-- Add unique constraint on name_norm if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'uq_locations_name_norm'
  ) THEN
...
BEGIN
  IF EXISTS (SELECT 1
    ...
    ALTER TABLE public.facts_monthly_location
      DROP CONSTRAINT facts_monthly_location_location_id_fkey;
    
    ALTER TABLE public.facts_monthly_location
      ADD CONSTRAINT facts_monthly_location_location_id_fkey 
...
BEGIN
  ...
  FOR i IN 1..n LOOP
    ...
    SELECT p_month, (SELECT id FROM loc_id LIMIT 1), v_vol, v_net, v_upload_id, now()
