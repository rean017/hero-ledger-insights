-- Fix unique constraints for existing tables structure

-- Add normalized column to existing locations table (using 'name' not 'dba')
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add unique constraint for case-insensitive location names
DO $$
BEGIN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'facts_monthly_location' 
    AND indexname = 'ux_fml_month_loc'
  ) THEN
