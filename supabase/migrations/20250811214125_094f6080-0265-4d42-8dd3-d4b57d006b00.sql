-- Create the bulk upload function using existing schema
...
begin
  ...
  with incoming as (
    select trim(p_locations[i]) as name, i
  ...
  -- 3) insert facts in bulk with upsert
  ...
  -- 4) get actual inserted count and zero-volume count
