...
stable
as $$
  select to_char(month, 'YYYY-MM') as month, count(*)::bigint as rows
  from public.facts_monthly_location
  group by to_char(month, 'YYYY-MM')
  order by month;
$$;

-- 2) Normalize any weird month strings into 'YYYY-MM'
create or replace function public.mh_fix_month_strings()
returns json
language plpgsql
as $$
declare
...
begin
  select json_agg(t) into before_counts
  from (
    select to_char(month, 'YYYY-MM') as month, count(*) as rows
    from public.facts_monthly_location
    group by to_char(month, 'YYYY-MM') order by month
  ) t;

  -- Since month is date type, we don't need string normalization
  -- Just count how many rows we have
  select count(*) into updated_rows from public.facts_monthly_location;

  select json_agg(t) into after_counts
  from (
    select to_char(month, 'YYYY-MM') as month, count(*) as rows
    from public.facts_monthly_location
    group by to_char(month, 'YYYY-MM') order by month
  ...
  return json_build_object(
    ...
    'after', after_counts,
    'message', 'Month column is date type, no normalization needed',
    'total_rows', updated_rows
  );
end;
$$;

-- 3) A bullet-proof Locations RPC with LEFT JOINs
...
with base as (
  ...
  where (p_month is null or to_char(f.month, 'YYYY-MM') = p_month)
),
locs as (
  select
    l.id as location_id,
