...
DROP VIEW IF EXISTS public.v_locations_month_with_flags CASCADE;
DROP VIEW IF EXISTS public.v_locations_month CASCADE;
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH exact AS (SELECT id FROM public.locations WHERE name = p_name),
       fuzzy AS (SELECT id FROM public.locations WHERE name ILIKE p_name)
  SELECT
    (SELECT id FROM exact LIMIT 1) as found_id,
    (SELECT count(*)::integer FROM exact) as exact_count,
    (SELECT count(*)::integer FROM fuzzy) as ilike_count;
$$;