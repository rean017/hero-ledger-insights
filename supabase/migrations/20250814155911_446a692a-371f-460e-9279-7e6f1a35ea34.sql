...
-- Add unique constraint on lowercase name if it doesn't exist
do $$
begin
  if not exists (
    select 1 from pg_indexes 
    where tablename = 'locations' 
    and indexname = 'locations_name_lower_unique'
  ) then
    create unique index locations_name_lower_unique on locations (lower(name));
  end if;
end $$;
...
-- remove ghost/demo tables that cause errors
