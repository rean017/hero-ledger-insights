...
-- Create locations table with proper unique constraint
...
begin
  if not exists (select 1 from information_schema.table_constraints 
                 where constraint_name = 'locations_name_lower_unique' 
                 and table_name = 'locations') then
    alter table locations add constraint locations_name_lower_unique unique (lower(name));
  end if;
end $$;
